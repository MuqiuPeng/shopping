 ...
 *
 * Records the event, evaluates trigger rules on coupons, and writes back the
 * granted couponId on the event row. Never throws — failures are recorded on
 * the event row so the calling webhook/action keeps responding 200.
 */
export async function emitCustomerEvent<T extends CustomerEventType>(
  ...
  const dedupeKey =
    input.dedupeKey ??
    buildDedupeKey(input.type, input.customerId, input.payload as Record<string, unknown>);
...
function computeValidityDays(couponEndDate: Date): number {
  const days = Math.floor((couponEndDate.getTime() - Date.now()) / 86_400_000);
