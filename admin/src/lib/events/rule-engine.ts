...
/**
 * Find coupons whose trigger settings match the given event + customer.
 * Filters out coupons the customer has already been granted (one grant per
 * customer per coupon).
 */
export async function findMatchingCoupons({
  ...
  for (const coupon of candidates) {
    if (
      !(await evaluateCondition(coupon.triggerCondition, eventType, payload, customer))
    )
      continue;
...
function asObject(json: Prisma.JsonValue | null): Record<string, unknown> | null {
